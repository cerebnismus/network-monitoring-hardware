/**
 * @file snmp.cpp
 * @details This program is used to send SNMP request and receive reply.
...
void read_communities(char *filename)
{
    ...
    if (o.debug > 0)
        fprintf(stdout, "Using community file %s\n", filename);
...
void read_hosts(char *filename)
{
    ...
    {
        if (o.debug > 0)
            fprintf(stdout, "Reading hosts from input file %s\n", filename);
        if ((fd = fopen(filename, "r")) == 0)
        {
    ...
    if (o.debug > 0)
        fprintf(stdout, "%d hosts read from file\n", host_count);
}
...
void init_options(int argc, char *argv[])
{
    ...
    if (!input_file)
    {
        ...
        if (o.debug > 0)
            fprintf(stdout, "Target ip read from command line: %s\n", argv[optind - 1]);
    }
    else
    {
    ...
    if (optind < argc)
    {
        ...
        community_count = 1;
        if (o.debug > 0)
            fprintf(stdout, "Community read from command line: %s\n", community[0]);
        ;
    }
    ...
    if (o.debug > 0)
        fprintf(stdout, "Waiting for %ld milliseconds between packets\n", o.wait);
}
...
void parse_snmp_response(u_char *buf, int buf_size)
{
    ...
    if (parse_snmp_objheader(buf, buf_size, &i) == -1)
        return;
    if (parse_snmp_objheader(buf, buf_size, &i) == -1)
        return; // yes, this should be called twice
...
int main(int argc, char *argv[])
{
    ...
    if (!o.quiet)
        fprintf(stdout, "Scanning %d hosts, %d communities\n", host_count, community_count);

    for (c = 0; c < community_count; c++)
    {
        if (o.debug > 0)
            fprintf(stdout, "Trying community %s\n", community[c]);
    ...
    if (o.debug > 0)
        fprintf(stdout, "All packets sent, waiting for responses.\n");
